...
public class TccTestController {
	...
	public static void tccBarrierTrans(Tcc tcc) throws Exception {
		...
		// 用户1 转出30元
		Response outResponse = tcc.callBranch(new TransReq(transOutUserId, -30), svc + "/barrierTransOutTry",
				svc + "/barrierTransOutConfirm", svc + "/barrierTransOutCancel");
		log.info("outResponse:{}", outResponse);
		// 用户2 转入30元
		Response inResponse = tcc.callBranch(new TransReq(transOutUserId + 1, 30), svc + "/barrierTransInTry",
				svc + "/barrierTransInConfirm", svc + "/barrierTransInCancel");
		log.info("inResponse:{}", inResponse);
	}
	...
	public static void tccBarrierTransError(Tcc tcc) throws Exception {
		// 用户1 转出100000元
		Response outResponse = tcc.callBranch(new TransReq(1, -100000), svc + "/barrierTransOutTry",
				svc + "/barrierTransOutConfirm", svc + "/barrierTransOutCancel");
		log.info("outResponse:{}", outResponse);
		// 用户2 转入100000元
		Response inResponse = tcc.callBranch(new TransReq(2, 100000), svc + "/barrierTransInTry",
				svc + "/barrierTransInConfirm", svc + "/barrierTransInCancel");
		log.info("inResponse:{}", inResponse);
	}
}