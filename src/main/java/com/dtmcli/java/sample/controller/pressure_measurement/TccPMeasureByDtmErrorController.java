...
@Slf4j
@RestController
@RequestMapping(("pmeasure/byDtm/error/"))
public class TccPMeasureByDtmErrorController {

	private static final String svc = "http://localhost:8081/pmeasure/byDtm/error";
    ...
    public Object TransInConfirm(HttpServletRequest request) throws Exception {
        BranchBarrier branchBarrier = new BranchBarrier(request.getParameterMap());
        log.info("barrierTransInConfirm branchBarrier:{}", branchBarrier);
        try (Connection connection = dataSource.getConnection()) {
        	TransReq transReq = extracted(request);
        	branchBarrier.call(connection, (barrier) -> {
